...
import { ComboConductores, ComboEmpresa, MostrarPaneles, MostrarBotones, TraeUsuario } from "./Globales.js"
...
const BtnEslaPersona = document.getElementById("BtnEslaPersona");
...
let CodConductorAux = document.getElementById('CodConductorAux');
...
const InicioSistemaServicios = async () => {
  ...
  LlenarServiciosLlistos(await TraerServiciosListos("servicioslistos/" + parseInt(CodConductorAux.value)));
...
BtnEslaPersona.addEventListener('click', async function () {
  // const UsuarioApp = document.getElementById("InpUsuarioApp");
  // const PasswordApp = document.getElementById("InpPasswordApp");
  // const DatosIncorrectos = document.getElementById("DatosIncorrectos");
  // const DivLogin = document.getElementById("DivLogin");
  // const DivApp = document.getElementById("DivApp");
  // let ArrayUsuario = [];
  // ArrayUsuario = await TraeUsuario("usuarios");
  // console.log(ArrayUsuario);
  // let ArrayPaso = ArrayUsuario.filter(Usuario => Usuario.email === UsuarioApp.value)
  // console.log(ArrayPaso);
  // let ok = "NO"
  // if(ArrayPaso.length > 0){
  //   if(ArrayPaso[0].password === PasswordApp.value){
  //     ok = "SI"
  //   }
  // }
  // if(ok === "NO"){
...
on(document, "click", "#btnSuma", (e) => {
  ...
  let codpasajero = fila.children[3].innerHTML;
  ...
  const pax = ArregloAux.find((pasajeros) => pasajeros.codpasajero === codpasajero);
  if (pax == undefined) {
    if (npax === 0) {
    ...
    indice = npax;
    AgregarPasajerosAux(codpasajero, nombre, celular, direccion, comuna, indice)
    MostrarPasajerosViajan("");
  }
});
...
on(document, "click", "#btnResta", (e) => {
  ...
  const index = ArregloAux.findIndex((pasajeros) => pasajeros.codpasajero === indice);
...
function AgregarPasajerosExistente(codpasajero, nombre, celular, direccion, comuna) {
  let nuevo = {
  ...
  try {
    ...
    data.forEach((item) => {
      ...
      <tr style="margin-top:0px;">
              ...
              <td> ${item.nombre} </td>
              <td> ${item.celular} </td>
              <td style="display: none"> ${item.codpasajero} </td>
...
function AgregarPasajerosAux(codpasajero, nombre, celular, direccion, nomcomuna) {
  let nuevo = {
    codpasajero: codpasajero,
    nombre: nombre,
    celular: celular,
...
const MostrarPasajerosViajan = (ok) => {
  ...
  ArrayPasajerosViajan.map((pax) => {
    ...
    const contenido = `
        ...
        <td style="display:none;">${pax.codpasajero}</td>`;
...
function CuantosPasajerosViajan() {
  ...
  ArrayPasajerosViajan.forEach((item) => {
    CRUDGrabaPasajerosViajan(item.codpasjaro)
  });
}
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
function EnvioEmail() {
  //CargarWhaysappEmial(ArrayServiciosListos);
  let body = "Es lo que queria..."
  let mensaje = "";
  let indice = 0;
  ArrayServiciosListos.forEach((servicio) => {
    mensaje = mensaje + servicio.fecha.substring(5, 10) + "   " + servicio.nombrecorto + "  " + servicio.nomtipo +""
...
// const CargarWhaysappEmial = (data) => {
//   try {
//     let mensaje = '';
//     let indice = 0;
//     let body = "";
...
//       mensaje += `
//           <tr>
//               <td> ${item.fecha.substring(5,10)} </td>
